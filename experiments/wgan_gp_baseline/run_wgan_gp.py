...
def gradient_penalty(critic, real, fake, y, device):
    alpha = torch.rand(real.size(0), 1, device=device)
    interp = alpha * real + (1 - alpha) * fake
    interp.requires_grad_(True)
    d_interp = critic(interp, y)
    grads = autograd.grad(
        outputs=d_interp,
        inputs=interp,
...
def main(cfg):
    ...
    c = Critic(x_dim, cfg["model"]["hidden_dim"], cfg["model"]["num_classes"]).to(device)
    opt_g = torch.optim.Adam(g.parameters(), lr=cfg["train"]["lr"], betas=(0.5, 0.9))
    opt_c = torch.optim.Adam(c.parameters(), lr=cfg["train"]["lr"], betas=(0.5, 0.9))

    for epoch in range(cfg["train"]["epochs"]):
        for xb, yb in train_loader:
            xb = xb.to(device).view(xb.size(0), -1)
            yb = yb.to(device)
            for _ in range(cfg["train"]["critic_steps"]):
                z = torch.randn(xb.size(0), cfg["model"]["z_dim"], device=device)
                fake = g(z, yb)
                c_real = c(xb, yb).mean()
                c_fake = c(fake.detach(), yb).mean()
                gp = gradient_penalty(c, xb, fake.detach(), yb, device)
                c_loss = -(c_real - c_fake) + cfg["train"]["gp_lambda"] * gp
                opt_c.zero_grad()
                c_loss.backward()
                opt_c.step()
            z = torch.randn(xb.size(0), cfg["model"]["z_dim"], device=device)
            ...
            g_loss.backward()
            opt_g.step()
        print(f"Epoch {epoch+1}/{cfg['train']['epochs']} c_loss={c_loss.item():.4f} g_loss={g_loss.item():.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="experiments/wgan_gp_baseline/configs/base.yaml")
